 ...
 */
private[made] def treeInfo(using quotes: Quotes)(tree: quotes.reflect.Tree): String =
  import quotes.reflect.*
  s"""
     |Structure ${Printer.TreeStructure.show(tree)}
     |ShortCode ${Printer.TreeShortCode.show(tree)}
     |""".stripMargin
private[made] def positionInfo(using quotes: Quotes)(pos: quotes.reflect.Position): String =
  s"""
     |start: ${pos.start},
     |end: ${pos.end},
...
inline private[made] def raiseCannotDerivedTypeFor[For <: AnyKind, Provided] = ${
